...
The cohort default lets `3dBlurToFWHM` select a subset of blurmaster volumes for speed. A documented one-run `-bmall` exception is permitted only when an explicit overwrite retry reproduces an out-of-tolerance all-volume audit result. The exception changes the convergence sample to all time points; it does not change the 6-mm target, analysis mask, input, output naming, or final independent audit.

RF1 `sub-10657`, session 01, run 1 is the sole approved exception. Its default overwrite retry achieved 5.28041 mm classic FWHM and its all-255-volume `-bmall` retry achieved 5.27996 mm, demonstrating a stable run-specific convergence limit. Its baseline was 3.12485 mm, so smoothing materially increased while stopping 0.12004 mm below the predeclared 5.4-mm lower QC bound. The final audit accepts this run only within a tightly bounded 5.27-5.29 mm range through `docs/smoothing_qc_exceptions.tsv`; it does not relax the cohort-wide 10% tolerance or waive geometry, file, mask, or other QC failures.
