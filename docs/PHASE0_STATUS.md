...
| Motion/coverage/outliers | Restartable run-level workflow and review-only flags implemented. Motion uses named fMRIPrep confounds rather than the headerless FEAT nuisance matrix. Coverage preserves the historical cerebellum/posterior-brainstem exemption through a fixed TemplateFlow-minus-exemption denominator; the full TemplateFlow mask remains the tSNR reference. Default flags match the dataset-specific 1.5×IQR tSNR/mean-FD/coverage scheme; fixed descriptive cutoffs are opt-in. Real 765-run corrected-coverage results pending. |
...
| Ratings | Modern source-resolved audit implemented. It requires all six partner×outcome cells, preserves cell means/counts and hashes, allows loss=win, and excludes subjects only for missing/empty data, identical ratings, or loss sum strictly greater than win sum. Source discovery and full-cohort results pending. |
