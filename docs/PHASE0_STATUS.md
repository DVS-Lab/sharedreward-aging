...
| Pooled temporal model | Approved and implemented as a deterministic narrow transform of the retained historical FSFs: nine full-trial task EVs, convolved optional missed-trial nuisance, 22 neutral-independent activation contrasts, zero FEAT smoothing, and no unsupported decision EVs. Neutral is modeled when observed and explicitly empty when absent; PPI uses the same 22 interaction contrasts plus the physiological contrast. See `NEUTRAL_CONDITION_DECISION.md`. |
