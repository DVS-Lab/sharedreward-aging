...
| Missed trials | Complete for all 758 runs with available source events: 19 runs exceed 25% missed trials, 27 have at least one zero-count modeled condition, and four subjects have no usable run (`ds003745`: 118, 135, 138; `rf1`: 12041). Seven RF1 imaging runs lack source events; these exactly match already-executed upstream source exclusions rather than new repair items. Final task manifests automatically exclude them and preserve valid opposite runs. |
| Ratings | Complete for 397 subjects with zero unresolved schema failures. Forty-nine subjects meet a ratings exclusion: 30 missing source files, 10 identical-rating cases, and nine strict loss-sum-greater-than-win-sum cases. The 11 missing ds003745 files are absent from the pinned OpenNeuro inventory and repository history. RF1 `sub-10803` is recoverable from Git history. Session-note review partitions the other RF1 gaps into confirmed non-collection/incompleteness, high-priority reported-complete recovery searches, and unresolved secondary searches; see `docs/SOURCE_GAPS.md` and `docs/rf1_ratings_source_notes.tsv`. |
| Cohort freezing | Implemented in `code/build_analysis_cohort.py`. It produces task-valid and ratings-qualified L1/L2 manifests separately, automatically applies the seven missing-event source exclusions, the 19 >25%-missed run exclusions, and curated task invalidations, and holds zero-count-condition runs for model review. RF1 `sub-11539` is excluded from task modeling because the wrong friend photo was displayed in both runs. |
...
4. run `code/build_analysis_cohort.py`, inspect the zero-count-condition holds, and freeze the generated run/subject disposition tables;
