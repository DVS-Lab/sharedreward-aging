...
| Ratings | Modern source-resolved audit implemented. RF1 sources are referenced from the authoritative tracked `rf1-sra` repository, and original-case headers are supported directly. It requires all six partner×outcome cells, preserves cell means/counts and hashes, allows loss=win, and excludes subjects only for missing/empty data, identical ratings, or loss sum strictly greater than win sum. One two-file source resolution and four repeated-header pre/post files are handled explicitly with provenance. The Linux2 audit remains pending because the retrieved ds003745 ratings content is not present in this local clone. |
