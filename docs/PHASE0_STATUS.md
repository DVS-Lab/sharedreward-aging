...
| Pilot achieved smoothing | Pending one operational 6-mm sanity check; no multi-kernel comparison is required. |
...
1. run one 6-mm operational sanity check from each dataset and verify achieved classic and ACF estimates;
2. launch restartable full-cohort target smoothing through `nohup` and `run_logged.sh`;
3. require complete achieved-smoothness QC before any L1 launch;
4. retain the target-encoded derivatives and keep FEAT smoothing at zero;
5. keep pooled L3 out of scope until dataset-specific L1/L2 outputs and QC are complete.
