...
class PooledFSFTest(unittest.TestCase):
    ...
    def test_activation_is_narrow_fulltrial_contract(self):
        ...
        self.assertEqual(setting(text, "conmask1_1"), "0")
        self.assertEqual(text.count("set fmri(conmask1_1)"), 1)
        ...
        self.assertEqual(setting(text, "conname_real.10"), '"rew-pun"')
        self.assertEqual(setting(text, "conname_real.27"), '"F-S (pun)"')
        self.assertEqual(setting(text, "conname_real.28"), '"F-C (pun)"')
        self.assertEqual(setting(text, "con_real8.8"), "1")
        for contrast in set(range(1, 29)) - {7, 8, 9, 20, 21, 22}:
    ...
    def test_ppi_uses_10_psych_phys_and_10_interactions(self):
        ...
        self.assertEqual(setting(text, "con_real8.19"), "1")
        for contrast in set(range(1, 29)) - {7, 8, 9, 20, 21, 22}:
