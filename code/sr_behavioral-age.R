...
data <- read.csv('/ZPOOL/data/projects/sharedreward-aging/code/combined_logs_with_age-nov12.csv')

# Create a new column for rew F-S
df <- data %>% 
  mutate(
    rew_FS = ifelse(
      partner == 3 & trait == 0,
      response - response[partner == 2 & trait ==0],
      NA
    )
  )

df <- df %>% select(c('sub', 'age', 'rew_FS'))
df <- na.omit(df)
